...
class BudgetRepository {
  ...
  Future<void> create({
    ...
    if (budget.walletId.trim() != wallet.id.trim() ||
        budget.createdByUserId.trim() != userId) {
      throw StateError('Dados do orçamento não pertencem ao contexto atual.');
    }
    await _referenceFor(
      wallet,
      userId,
    ).doc(budget.id).set(BudgetModel.toMap(budget));
  }
  ...
  Future<List<Budget>> getByWallet({required WalletModel wallet}) async {
    ...
    final budgets =
        snapshot.docs
            .map(
              (document) =>
                  BudgetModel.fromMap(document.data(), documentId: document.id),
            )
            .where((budget) => budget.walletId == wallet.id)
            .toList()
  ...
  Future<Budget> update({
    ...
    return _firestore.runTransaction((transaction) async {
      ...
      final persisted = BudgetModel.fromMap(
        document.data()!,
        documentId: document.id,
      );
      if (!wallet.isShared && persisted.createdByUserId != userId) {
        throw StateError(
          'Somente o responsável pelo orçamento pode alterá-lo.',
        );
      }
      if (persisted.walletId != wallet.id)
        throw StateError('O orçamento não pertence à carteira.');
  ...
  Future<Budget> changeStatus({
    ...
    return update(
      budget: budget.copyWith(status: status, updatedAt: DateTime.now()),
      wallet: wallet,
    );
  }

  CollectionReference<Map<String, dynamic>> _referenceFor(
    WalletModel wallet,
    String userId,
  ) {
    if (wallet.isShared)
  ...
  void _validateWalletAccess(WalletModel wallet, String userId) {
    if (!wallet.hasMember(userId))
      throw StateError('Usuário sem acesso à carteira do orçamento.');
  }
