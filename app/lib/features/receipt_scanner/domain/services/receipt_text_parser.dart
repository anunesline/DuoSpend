...
class ReceiptTextParser {
  ...
  List<ReceiptScanItem> _findItems(List<String> lines) {
    ...
    for (final line in lines) {
      final upper = line.toUpperCase();
      if (upper.contains('TOTAL') || upper.contains('CNPJ') || upper.contains('PIX')) {
        continue;
      }
      ...
      if (quantityMatch != null) {
        ...
        if (quantity != null && unitPrice != null && totalPrice != null) {
          items.add(ReceiptScanItem(
            description: quantityMatch.group(1)!.trim(),
            quantity: quantity,
            unitPrice: unitPrice,
      ...
      final totalOnlyMatch = totalOnlyPattern.firstMatch(line);
      if (totalOnlyMatch == null) continue;
      final totalPrice = _parseMoney(totalOnlyMatch.group(2)!);
      final description = totalOnlyMatch.group(1)!.trim();
  ...
  double? _parseMoney(String value) {
    ...
    if (decimalPlaces <= 0) return double.tryParse(digits);
    return double.tryParse(digits) == null
        ? null
        : double.parse(digits) / _powerOfTen(decimalPlaces);
  }
